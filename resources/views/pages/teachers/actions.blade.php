    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h4 class="modal-title"> {{ __('Delete teacher') }} </h4>
          <button type="button" class="close" data-dismiss="modal">&times;</button>
        </div>
        <div class="modal-body">
          {!! __('Are you sure want to delete teacher <b>:name</b> ?', ['name' => $name]) !!}
        </div>
        <div class="modal-footer">
          <form action="{{route('teacher.destroy', $id) }}" method="POST" id="delete-role-{{ $id }}">
            @csrf
            @method('DELETE')
